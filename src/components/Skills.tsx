...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 lg:py-24 bg-gradient-to-b from-slate-950 to-slate-900">
          ...
          {skillCategories.map((category, index) => {
            ...
            return (
              <div key={index} className="group relative bg-slate-900/50 backdrop-blur-xl rounded-3xl p-6 lg:p-8 border border-slate-800/50 hover:border-slate-700/50 transition-all duration-500 hover:transform hover:scale-105">
                <div className={`absolute inset-0 bg-gradient-to-br ${category.gradient} opacity-0 group-hover:opacity-5 rounded-3xl transition-opacity duration-500`}></div>
                
                <div className="relative z-10">
                  <div className="flex items-center gap-4 mb-8">
                    <div className={`flex items-center justify-center w-12 h-12 bg-gradient-to-br ${category.gradient} bg-opacity-10 rounded-2xl group-hover:scale-110 transition-transform duration-300`}>
                      <Icon className={`w-6 h-6 bg-gradient-to-br ${category.gradient} bg-clip-text text-transparent`} />
                    </div>
                    <h3 className="text-xl lg:text-2xl font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:to-blue-500 transition-all duration-300">
                      {category.title}
                    </h3>
                  </div>
                  ...
                  <div className="space-y-6">
                    {category.skills.map((skill, skillIndex) => (
                      <div key={skillIndex} className="group/skill">
                        ...
                        <div className="relative w-full bg-slate-800/50 rounded-full h-2 lg:h-3 overflow-hidden">
                          <div 
