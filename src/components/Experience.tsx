...
const Experience: React.FC = () => {
  ...
  return (
    <section id="experience" className="py-16 lg:py-24 bg-gradient-to-b from-slate-900 to-slate-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="absolute left-6 lg:left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-cyan-400 via-blue-500 to-purple-600 lg:transform lg:-translate-x-0.5 rounded-full"></div>
          ...
          <div className="space-y-12 lg:space-y-16">
            {experiences.map((exp, index) => (
              <div key={index} className={`relative flex items-start ${
                ...
                <div className="absolute left-6 lg:left-1/2 w-6 h-6 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full border-4 border-slate-950 lg:transform lg:-translate-x-1/2 z-10 shadow-lg shadow-cyan-500/50">
                  {exp.isActive && (
                    <>
                      <div className="absolute inset-0 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full animate-ping"></div>
                      <Zap className="absolute inset-0 w-3 h-3 text-white m-auto animate-pulse" />
                    </>
                  )}
                </div>
                ...
                <div className={`w-full lg:w-5/12 ml-16 lg:ml-0 ${
                  ...
                  <div className="group relative bg-slate-900/50 backdrop-blur-xl rounded-3xl p-6 lg:p-8 border border-slate-800/50 hover:border-slate-700/50 transition-all duration-500 hover:transform hover:scale-105">
                    <div className={`absolute inset-0 bg-gradient-to-br ${exp.gradient} opacity-0 group-hover:opacity-5 rounded-3xl transition-opacity duration-500`}></div>
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                        <div className="flex items-center gap-3 mb-2 sm:mb-0">
                          ...
                          <h3 className="text-xl lg:text-2xl font-bold text-white">{exp.company}</h3>
                          {exp.isActive && (
                            <span className="px-3 py-1 bg-gradient-to-r from-cyan-400 to-blue-500 text-white text-xs font-bold rounded-full animate-pulse">
                              CURRENT
                            </span>
                          )}
                        </div>
                      ...
                      <h4 className="text-lg lg:text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 mb-4">
                        {exp.role}
                      </h4>
                      
                      {/* Meta info - mobile optimized */}
                      <div className="flex flex-col gap-3 mb-6 text-sm text-slate-400">
                      ...
                      <ul className="space-y-3">
                        {exp.achievements.map((achievement, achievementIndex) => (
                          <li key={achievementIndex} className="flex items-start gap-3 text-sm lg:text-base text-slate-300">
